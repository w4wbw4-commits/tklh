...
import { Download, Loader2, Sparkles, Share2 } from "lucide-react";
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
export const InvitationDialog = ({ open, onOpenChange, event }: Props) => {
  ...
  const dateLabel = new Date(event.event_date).toLocaleDateString("ar-SA", {
    weekday: "long",
    year: "numeric",
    month: "long",
    day: "numeric",
  ...
  const generateAiCard = async () => {
    ...
    try {
      ...
      if (data?.image) {
        ...
        toast.success("تم تصميم بطاقتك");
      } else {
        throw new Error("لم يتم استلام صورة");
      }
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : "تعذر التصميم";
      toast.error(msg);
    } finally {
      setGenerating(false);
    }
  ...
  const downloadTemplate = async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `invitation-${event.id}.png`;
      a.click();
  ...
  const downloadAi = () => {
    ...
    const a = document.createElement("a");
    a.href = aiImage;
    a.download = `invitation-ai-${event.id}.png`;
    a.click();
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle className="font-arabic">بطاقة الدعوة الرقمية</DialogTitle>
          <DialogDescription>اختر قالباً جاهزاً أو دع الذكاء الاصطناعي يصمّم بطاقة فريدة.</DialogDescription>
        </DialogHeader>

        <Tabs defaultValue="template" className="w-full">
          <TabsList className="grid w-full grid-cols-2 rounded-full bg-secondary/60">
            <TabsTrigger value="template" className="rounded-full">قالب أنيق</TabsTrigger>
            <TabsTrigger value="ai" className="rounded-full">
              <Sparkles className="me-1 h-3.5 w-3.5" /> صمم بـ AI
            </TabsTrigger>
          </TabsList>
              ...
              >
                ...
                <div className="relative flex h-full flex-col items-center justify-between text-center">
                  <div>
                    ...
                    <div className="mt-3 font-arabic text-3xl font-bold leading-tight">
                      {event.title}
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <div className="font-arabic text-sm text-primary-foreground/85">
                      يسرّنا حضوركم
                    </div>
                    <div className="font-arabic text-base font-medium">{dateLabel}</div>
                    {event.city && (
                      <div className="text-xs text-primary-foreground/75">{event.city}</div>
                    )}
                    <div className="h-px w-16 bg-primary-foreground/40" />
                  </div>

                  <div className="text-[10px] uppercase tracking-[0.3em] text-primary-foreground/65">
                    تكلّة • Tekillah
                  </div>
                </div>
              </div>
            ...
            <div className="flex flex-col items-center gap-4">
                ...
                <div className="grid aspect-[3/4] w-full max-w-xs place-items-center rounded-3xl border border-dashed border-border bg-card p-6 text-center text-sm text-foreground/60">
                  {generating ? (
                    <div className="space-y-3">
                      ...
                      <div>جارٍ تصميم بطاقتك بالذكاء الاصطناعي…</div>
                    </div>
                  ) : (
                    "اضغط زر التصميم لإنشاء بطاقة فريدة"
                  )}
                </div>
              )}
              ...
              <div className="flex gap-2">
                ...
                >
                  ...
                  {aiImage ? "أعِد التصميم" : "صمم بطاقتي"}
                </Button>
                {aiImage && (
                  <Button onClick={downloadAi} variant="outline" className="rounded-full">
                    <Download className="me-1 h-4 w-4" /> تحميل
                  </Button>
                )}
              </div>
            </div>
