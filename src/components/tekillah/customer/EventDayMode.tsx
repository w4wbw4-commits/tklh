...
const liveStatuses = [
  { label: "بانتظار الوصول", classes: "bg-secondary text-foreground/65" },
  { label: "في الطريق", classes: "bg-amber-100 text-amber-700" },
  { label: "وصل ✓", classes: "bg-primary/10 text-primary" },
  { label: "جاهز للحفل", classes: "bg-primary/15 text-primary" },
];
...
export const EventDayMode = ({ event }: { event: EventRow }) => {
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <h3 className="font-arabic text-xl font-semibold sm:text-2xl">
                مباشر الآن — كل التحديثات لحظة بلحظة
              </h3>
            </div>
          </div>
      ...
      <div>
        <h4 className="mb-3 font-arabic text-lg font-semibold">حالة المزوّدين الآن</h4>
        {bookings.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-border bg-card p-8 text-center text-sm text-foreground/65">
            لا توجد حجوزات مؤكدة لعرض حالتها يوم الحفل.
          </div>
        ) : (
                ...
                >
                  ...
                  <div className="flex-1">
                    ...
                    <div className="mt-1 h-1.5 w-full overflow-hidden rounded-full bg-secondary">
                      <motion.div
                        animate={{ width: `${((progress[b.id] ?? 0) / 3) * 100}%` }}
                  ...
                  <span
                    className={`rounded-full px-2.5 py-1 text-[11px] font-medium ${status.classes}`}
                  >
                    {status.label}
                  </span>
                </motion.div>
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        ...
        >
          ...
          <div>
            <div className="font-arabic text-sm font-semibold text-primary">
              منسّق تكلّة الميداني
            </div>
            <div className="mt-0.5 text-xs text-foreground/65">اتصل الآن للدعم اللحظي</div>
          </div>
        </a>
        ...
        >
          ...
          <div>
            ...
            <div className="mt-0.5 text-xs text-foreground/65">للحالات العاجلة فقط</div>
          </div>
        </a>
      </div>
    </div>
